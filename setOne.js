...
// CODE HERE
const testfunk = (parm1, parm2) => console.log(`this is ${parm1}, this is ${parm2}.`)

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
// testfunk(`test1`, `test2`)
...
const fullName = (fullName) => fullName = console.log(`hello my full name is ` + fullName)

const greeting = (firstName, lastName, callBack) => {
    let fullName = firstName + ` ` + lastName
    callBack(fullName)
}
greeting(`Kees`, `Nauta`, fullName)
// const invokeGreeting = (`Kees`, `Nauta`, fullName)
...
const newFunk = totalCost => console.log(totalCost)

const pricesPlusTax = (arr, callback) =>{
    for(let i = 0; i < arr.length; i++){
        let totalPlusTax = arr[i] + (arr[i] * .2)
        totalCost.push(totalPlusTax)
    }
    callback(totalCost)
}
/* 
...
const multiplyingFactory = parm1 =>{
    return (parm2) => {
        if(parm1 >= 5){
